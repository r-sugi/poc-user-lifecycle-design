...
| 複数 DB 更新 | 対話的 Tx | D1 `db.batch([...])`（楽観ロックは単発 UPDATE → batch の2段階） |
...
| **Repository** | `repositories/*.ts` | D1/Drizzle 永続化を集約単位でラップ。「単発 UPDATE → 行数確認 → batch」の実装詳細はここに閉じ込め、呼び出し側には「原子的に状態遷移する」IF だけを見せる |
...
| 3 | 理由の分からない BAN | `user_bans.reason_code` / `reason_text` |
| 4 | 解約理由が取れない | `user_withdrawals` を退会時に必須 INSERT |
...
| シード固定ラベル（POC） | `seed_user_labels` / `seed_signup_labels` | 通常は消えない（再シード時のみ入れ替え）。操作で変わらない |
...
- 履歴は共通項目のみ。種別固有は `event_id` PK/FK の詳細テーブル
- トークン生値は保存しない
...
| verified_at | text | NOT NULL | 初回検証完了時刻 |
...
| email | text | NOT NULL | 現行メール。**UNIQUE は「有効プロフィール行間」で取る**（インデックスは通常 UNIQUE。退会後削除で再登録可能） |
| display_name | text | NOT NULL | |
| created_at / updated_at | text | NOT NULL | |

運用: 猶予中はプロフィールが残るため同一 email の新規登録は拒否（要件定義書の運用ルール）。PII 削除後は再登録可（別 user_id）。
...
| provider_uid | text | NOT NULL | password は user_id または固定識別子、google は `sub` |
...
#### `user_withdrawals` / `user_bans`

| テーブル | PK | 追加列 |
|---|---|---|
| user_withdrawals | event_id → user_status_events.id | reason_code, reason_text, created_at |
| user_bans | event_id → user_status_events.id | admin_user_id FK→admin_users, reason_code, reason_text, created_at |

#### `seed_user_labels` / `seed_signup_labels`（POC 検証トップ用・シード専用）

検証トップの **「初期状態」** 列用。ドメインの status とは独立した固定スナップショット。シード時のみ INSERT し、アプリの通常処理では UPDATE / DELETE しない（再シード時のクリアは可）。

| テーブル | PK | 列 |
|---|---|---|
| `seed_user_labels` | `user_id` → `users.id` | `initial_state_label` TEXT NOT NULL, `created_at` TEXT NOT NULL |
| `seed_signup_labels` | `signup_verification_id` → `signup_verifications.id` | `initial_state_label` TEXT NOT NULL, `raw_token` TEXT（POC・検証トップの認証リンク用・シード時のみ）, `created_at` TEXT NOT NULL |
...
| withdrawn | active | 退会取消（猶予内） | withdraw_cancelled |
| active | banned | BAN | banned |
| banned | active | BAN 解除 | unbanned |

`UserStatus.canTransitionTo(to)` で上記以外を拒否。withdrawn 中の BAN 等は本 PoC では未サポート（必要なら別途定義）。
...
D1 batch 内では「UPDATE 0件」を検知して他 statement を取り消せないため、順序を組み替える。

1. **単発** `UPDATE users SET status=?, last_seq=? WHERE id=? AND last_seq=?`
2. 影響行数 0 → 競合エラーで即座中断
3. 成功時のみ `db.batch([ events INSERT, 詳細 INSERT, ... ])`
4. **D1 成功後**、必要なら Queue に `{ userId, reason }` を投入（セッション破棄が必要な場合）

原設計の「履歴が正で status はキャッシュ」「直 UPDATE で履歴の穴を作らない」意図は維持する。変更点は **楽観ロック UPDATE を先に確定してから履歴を積む**ことと、セッション破棄を Tx 外の Queue に移したこと。
...
| 検証（middleware） | Cookie → hash → KV GET。**成功パスでは毎回 D1 を読まない** |
| ログアウト | **例外**: Queue を介さず当該1キーを即時 DELETE + Cookie 削除 |
| 一括失効 | D1 コミット後 Queue 投入 → consumer が prefix list → 順次 DELETE（冪等） |

### 8.6 一括失効トリガー

Queue メッセージ例: `{ "userId": "...", "reason": "ban" | "withdraw" | "password_change" | "password_reset" }`

| 契機 | API | reason |
|---|---|---|
| BAN | `POST /admin/users/:id/ban` | `ban` |
| 退会 | `POST /me/withdraw` | `withdraw` |
| パスワード変更 | `PUT /me/password` | `password_change` |
| パスワードリセット実行 | `POST /auth/password/reset` | `password_reset` |
...
| 項目 | 仕様 |
|---|---|
| パスワードハッシュ | PBKDF2-SHA256、反復 100,000、salt 16byte、保存形式 `pbkdf2$iter$salt$hash`（salt/hash は hex または base64 で統一） |
| パスワード最小長 | 8 |
...
- 処理: 既存 active プロフィールの同一 email があれば conflict。`signup_verifications` INSERT（有効な旧トークンは無効化してから新規）。メールログ出力
- レスポンス: `{ ok: true }`（トークンは返さない）
...
- 処理（方針 A）:
  ...
  2. batch: users(active, last_seq=1) / profile / identity(password) / event(activated, seq=1) / verification consumed
  3. セッション発行（KV + Cookie）
- 二重消費は拒否
...
- Body: `{ email }`
- 未消費の verification を無効化し新規発行、メールログ。存在しない email でもタイミング攻撃緩和のため同じ応答形でもよい（PoC では簡潔に 404/ok を設計実装で統一）

#### `POST /auth/login`

- Body: `{ email, password }`
- 処理: profile → identity(password) → user。`banned`/`withdrawn` はログイン拒否。成功時セッション発行
...
- Body: `{ email }`
- password_resets 発行、メールログ。登録無しでも一律 ok 応答可
...
- users + profile（+ 必要なら identities 概要）

#### `PATCH /me`

- Body: `{ displayName }` 等。**画面化しない**が API は提供
...
- Body: `{ newEmail }` → email_change_requests、確認メールログ
...
- 方針 B: withdrawn→active、event。セッション Queue は不要
...
- Body: `{ reasonCode, reasonText? }`
- 方針 B + **Queue: ban** + admin_user_id 記録
...
3. `email`/`name`/`sub`(email から派生した安定ダミー) を JSON 化し、base64url 等で `code` に埋め込み、`GET /auth/google/callback?code=...` へ 302
4. callback 処理（原設計 §11）:
   1. `(google, sub)` で identities 検索
   2. 無ければ `user_profiles.email` で既存検索  
      - あり → その user に google identity 追加  
      - なし → users 一式新規（方針 A）+ activated event
   3. 同メールの未消費 signup_verifications を無効化
   4. セッション発行
5. 識別子は **sub**（email ではない）。Google 側メール変更でも profile.email は自動上書きしない
6. 同一メール紐付けは「既に検証済みプロフィールが存在する」場合に自動
...
| Job | 内容 |
|---|---|
| `PurgeWithdrawnPiiUseCase` | `status=withdrawn` かつ退会イベントから猶予日数経過したユーザーの `user_profiles` / `user_identities` を DELETE。`reason_text` に PII が混ざる前提がある場合の扱いは「本 PoC では events/詳細は残す（マスクしない）」 |
...
| 退会 / 退会取消 / ログアウト | `POST /mypage/withdraw` 等 | ユーザー | 同上＋Route |
...
| `withdrawn / 退会後30日未満（取消可・PII残）` / 同・残りN日 | U06。または U01 退会直後。退会 event から **PII 削除待機（既定 30 日）** 未満 → 退会取消可・PII 残存。現在の状態は残り日数を付与可 |
...
操作: 退会、退会取消（withdrawn 時）、ログアウト、パスワードリセット申請へのリンク、メール変更申請へのリンク。
...
成功パスでは D1 の status 再確認を必須としない（KV のみ）。BAN 直後の最大60秒は古いセッションが通り得ることを仕様として受け入れる。`GET /me` / マイページ UseCase 側では status を読み、ban/withdrawn ならログアウト誘導してよい。
...
- [ ] 楽観ロック更新が batch 先頭に埋まっていないこと（単発→batch）
...
- [ ] `seed_user_labels` / `seed_signup_labels` がシード時のみ INSERT され、通常操作では更新されないこと
...
| A3 | `admin3@example.com` | BAN 解除ログ用 | `AdminPass123!` |
| A4 | `admin4@example.com` | 閲覧・検索確認用 | `AdminPass123!` |
| A5 | `admin5@example.com` | 予備 | `AdminPass123!` |

`admin_users` に `display_name` 列は無い。email と password_hash のみ。A1 を BAN イベントの既定 `admin_user_id` とする（解履歴の unban も同様に A1 または A3 でよい）。
...
| U06 | `frank.withdrawn@example.com` | Frank Withdrawn | `withdrawn` | password | `activated` → `withdrawn`（退会イベント = 基準日の 3 日前＝PII削除待機 30 日未満）。`user_withdrawals.reason_code=no_longer_needed`。プロフィール残存。セッションは投入しない（失効後想定） | `withdrawn / 退会後30日未満（取消可・PII残・残り27日）` | 退会取消可能・PII 残 |
...
| U10 | `jack.cancelled@example.com` | Jack Cancelled | `active` | password | `activated` → `withdrawn` → `withdraw_cancelled`（`user_withdrawals` は退会 event に紐づく）。現 status=`active` | `active / 退会取消済み（履歴あり）` | 退会取消後の復帰 |
...
| U14 | `noah.unbanned@example.com` | Noah Unbanned | `active` | password | `activated` → `banned`（`tos_violation`）→ `unbanned`。現 status=`active` | `active / BAN解除済み（履歴あり）` | BAN 解除履歴あり |
