...
| 複数 DB 更新 | 対話的 Tx | D1 `db.batch([...])`（event INSERT・詳細 INSERT・users UPDATE を単一 batch。UNIQUE 違反で batch 全体ロールバック） |
...
| **Repository** | `repositories/*.ts` | D1/Drizzle 永続化を集約単位でラップ。「event INSERT・詳細 INSERT・users UPDATE の単一 batch」の実装詳細はここに閉じ込め、呼び出し側には「原子的に状態遷移する」IF だけを見せる |
...
> **実装時の統合方針**: 当初の設計では機能単位で1ファイル1クラスとする想定だったが、実装では関連する複数 UseCase / Repository を1ファイルに複数 export する形（例: `AdminUseCases.ts`, `EmailChangeUseCases.ts`）へ多くを統合している。下記は実ファイル構成であり、クラス名は変わっていない（`container.ts` の import 元を参照）。
...
/
  ...
  scripts/
    seed.ts                      # ローカル用シード（admin 5 / user 16 / signup_verifications 別枠5行）
    queue-smoke.ts                # Queues ローカル疎通確認用の手動スクリプト
    verify-withdraw-cancel.ts     # 退会→取消シナリオの手動確認スクリプト
  src/
    index.tsx                    # Hono app、ルーティング、scheduled / queue ハンドラ export（JSX を直接使うため .tsx）
    ...
    domain/
      ...
      shared/
        ...
        StatusEventEnums.ts          # event type / actor_type の定数（設計書に未記載だった追加分）
    queries/
      UserEventTimelineQuery.ts      # 管理者詳細画面用の履歴 JOIN クエリ（Repository と分離）
    repositories/
      UserRepository.ts
      ...
      UserStatusEventRepository.ts    # UserStatusEventRepository と UserBanRepository の実体をここに定義
      UserBanRepository.ts            # ↑からの re-export シムのみ
      StatusTransitionWriter.ts       # event + 詳細(withdrawals/bans/unbans) + users を単一 batch で書く（§7.2）。
                                       # 専用の UserWithdrawalRepository は無く、withdrawals の書き込みはここに一本化されている
      SeedLabelRepositories.ts        # SeedUserLabelRepository・SeedSignupLabelRepository を1ファイルに定義
    gateways/
      MailerGateway.ts
    ...
    services/
      ...
      DisableAdminService.ts            # 管理者 disable 時の同期セッション失効（§8.6）
    usecases/
      signup/
        SignupUseCase.ts
      ...
      passwordReset/
        PasswordResetUseCases.ts     # RequestPasswordResetUseCase・ResetPasswordUseCase を1ファイルに統合
      emailChange/
        EmailChangeUseCases.ts       # RequestEmailChangeUseCase・VerifyEmailChangeUseCase・ChangePasswordUseCase を統合
                                      # （ChangePasswordUseCase は当初 me/ 配下の想定だったが実装ではここに置かれている）
      me/
        GetMeUseCase.ts              # GetMeUseCase・UpdateProfileUseCase（PATCH /me。画面なし）を統合
      withdraw/
        WithdrawUseCases.ts          # WithdrawUseCase・CancelWithdrawUseCase を統合
      admin/
        AdminUseCases.ts             # AdminLoginUseCase・SearchUsersUseCase・GetUserDetailUseCase・BanUserUseCase・UnbanUserUseCase を統合
        ListVerificationHomeUseCase.ts  # 検証トップ（GET /）の一覧合成。設計書に未記載だった追加分（§16.2 参照）
      dev/
        DevLoginAsUseCase.ts         # 検証トップ「ログイン」用。パスワード不要でユーザーセッション発行（production では拒否）
        DevLoginAsAdminUseCase.ts    # 管理者版。同上（設計書に未記載だった追加分）
      batch/
        BatchUseCases.ts             # PurgeWithdrawnPiiUseCase・PurgeExpiredTokensUseCase（cron/内部バッチ）
                                      # + ForcePurgeUserPiiUseCase・ForceAnonymizeUserPiiUseCase（管理者操作 §11.3）を統合
    presentation/
      api/
        auth.ts                     # signup / signup/verify / signup/resend / login / logout / google / google/callback
                                      # / password/reset-request / password/reset / email/verify をまとめて実装
                                      # （設計書旧稿の signup.ts / login.ts / google.ts / passwordReset.ts / emailChange.ts に相当）
        me.ts                       # GET・PATCH /me, PUT /me/password, POST /me/email, POST /me/withdraw
        admin.ts
        internalBatch.ts
      pages/
        homePage.tsx                # GET /（POC検証トップ。認証不要）
        appPages.tsx                 # signupPage・mypagePage・adminUsersListPage・adminUserDetailPage・signupVerifyPage
        extraPages.tsx                # googleConsentPage・passwordResetRequestPage・passwordResetPage
                                      # ・emailChangePage・emailChangeVerifyPage
        adminLoginPage.tsx            # ※ パスワード入力フォームではない。§16.1 の注記を参照
                                      # （設計書旧稿の loginPage.tsx / googleMockConsentPage.tsx / passwordResetPages.tsx
                                      #   / emailChangePages.tsx / adminUsersListPage.tsx / adminUserDetailPage.tsx は
                                      #   上記3ファイルへ統合。loginPage.tsx に相当する画面は実装されていない）
      schemas/                   # valibot スキーマ（リクエスト／フォーム単位）
        ...
        admin.ts
      components/
        statusBadge.tsx           # StatusBadge・StateChip（「現在の状態」チップ）
        eventTimeline.tsx
    renderer.tsx                # hono/jsx-renderer + ViteClient/Script（presentation/ 配下ではなく src/ 直下）
    middleware/
      requireUser.ts
      requireAdmin.ts
      requireUserPage.ts
    ...
    lib/
      flash.ts
      errors.ts
      assertUserActive.ts         # /me 系入口の active チェック共通化
      dbErrors.ts                 # D1 UNIQUE 違反判定（isUniqueViolation）
      ids.ts                      # newId() 等の ID 生成
      appEnv.ts                   # isDevLoginEnabled() 等の APP_ENV 分岐
      datetime.ts                 # formatJst() 等
      validate.ts                 # parseOrThrow()（valibot ラッパー）
```
...
| display_name | text | NOT NULL | 検証完了時に `user_profiles.display_name` へ引き継ぐ。アプリ必須データのため本体に保持（旧: `seed_signup_labels` に混在していたが分離） |
...
**用語注記（anonymize）**: 匿名化後の email は `anon-<userId>@anon.invalid` とする。元メールは消えるが、内部識別子（user_id）は残る。履歴を残したまま連絡先だけ落とす運用向け。
...
2. 1 文目の `UNIQUE(user_id, seq)` 違反 → 例外 → **batch 全体ロールバック** → 409 `optimistic_lock_conflict`（後勝ちさせない。冪等キーは持たない）
...
- `purge-pii` / `anonymize-pii` は猶予を見ずにいつでも実行できるため、取消の直前に `user_profiles` が存在し、かつ匿名化済み（`email` が `@anon.invalid`）でないことを確認する。どちらかに該当する場合は `pii_already_removed`（400）を返す。このチェックが無いと「`status = active` だが profile / identity が無い（またはメールが匿名化済みの）ユーザー」が生まれ、ログインもマイページ表示もできなくなる
...
### 11.4 検証トップ専用エンドポイント（実装の現況。設計時は未記載）

パスワード認証を経ずにセッションを発行する、**ローカル POC 専用**のエンドポイント。`APP_ENV=production` では 404 / 403（`isDevLoginEnabled`）。JSON API ではなく `src/index.tsx` に直接登録された画面用 POST ハンドラで、PRG + flash で結果を返す。

| メソッド / パス | 説明 |
|---|---|
| `POST /dev/login-as` | Body: `{ userId, redirect? }`。対象ユーザーのセッションを即発行（`DevLoginAsUseCase`）。`banned` / `withdrawn` は拒否。検証トップの確定ユーザー行「ログイン」ボタンが呼ぶ |
| `POST /dev/login-as-admin` | Body: `{ adminId, redirect? }`。対象管理者のセッションを即発行（`DevLoginAsAdminUseCase`）。`disabled_at` 設定済みは拒否。`/admin/login` 画面の管理者一覧「ログイン」ボタンが呼ぶ |
| `POST /dev/logout-admin` | 管理者セッション削除（画面用。JSON API の `POST /admin/logout` とは別経路） |
| `POST /signup/resend` | Body: `{ email, redirect? }`。検証トップの未認証一覧「再送」ボタン用（画面版）。処理自体は `POST /auth/signup/resend` と同じ `ResendSignupVerificationUseCase` を呼ぶ |

これらは §16.2 の「検証トップからパスワード無しでログイン可能にする」要件を満たすための実装であり、**§9 のパスワード認証（PBKDF2）を回避する経路が本番以外に存在する**ことを意味する。`isDevLoginEnabled` による `APP_ENV` 分岐が唯一の安全装置であるため、本番相当の環境変数投入時にこの分岐が正しく効くことは実装上の必須確認事項とする。
...
| 409 | `conflict` / `status_conflict` / `optimistic_lock_conflict` | email 重複、楽観ロック失敗、不正遷移 |
| 410 | `token_expired` / `token_consumed` | トークン期限切れ・消費済み |
...
| Job | 内容 |
|---|---|
| `PurgeWithdrawnPiiUseCase` | `status=withdrawn` かつ、当該ユーザーの **最大 seq の `withdrawn` イベント**の `created_at` から猶予日数（既定 30 日）経過したユーザーの `user_profiles` / `user_identities` を DELETE。候補抽出は `user_profiles` への INNER JOIN を含む（既に purge 済み＝profile 無しのユーザーを候補から除外するため。これが無いと `LIMIT 50` が purge 済み行で埋まり続け、51人目以降に永久に到達できなくなる）。退会→取消→再退会の場合は**再退会イベント**が基準。`reason_text` に PII が混ざる前提がある場合の扱いは「本 PoC では events/詳細は残す（マスクしない）」 |
| `PurgeExpiredTokensUseCase` | signup_verifications / password_resets / email_change_requests の消費済みまたは期限切れ行を DELETE。`signup_verifications` 側は `seed_signup_labels` を同一 batch で先に削除（FK 順序）。**既知の制約**: 対象 ID 収集に `LIMIT` を付けていないため、D1 の 1 クエリあたりバインドパラメータ上限（100）を超える件数が同時に期限切れ/消費済みになると `inArray` の DELETE が失敗する。ローカル検証の件数では表面化しない想定。本番投入前は `PurgeWithdrawnPiiUseCase` の `PURGE_BATCH_LIMIT` と同様に `.limit()` を付けて cron で反復する形に揃えること |
...
> **設計との差分**: 「ログイン」「管理者ログイン」は当初パスワード入力フォームの画面として設計したが、実装では **パスワード画面を作らず**、検証トップ／管理者一覧からのワンクリック impersonate（`/dev/login-as` 系）に置き換えている。パスワード認証そのものは JSON API（`POST /auth/login` / `POST /admin/login`）としては実装されているが、これを叩く画面（`loginPage.tsx` 相当）は存在しない。

| 画面 | メソッド / パス | 認証 | 実装ファイル（目安） |
|---|---|---|---|
| **検証トップ** | `GET /` | 不要 | `presentation/pages/homePage.tsx` |
| サインアップ | `GET/POST /signup` | 不要 | `appPages.tsx`（`signupPage`） |
| サインアップ検証結果 | `GET /auth/signup/verify` | 不要 | `appPages.tsx`（`signupVerifyPage`） |
| サインアップ確認メール再送（検証トップ用） | `POST /signup/resend` | 不要 | `index.tsx`（§11.4） |
| ~~ログイン（パスワード画面）~~ | ~~`GET/POST /login`~~ | — | **未実装**。パスワードログインは `POST /auth/login`（JSON API のみ）で提供 |
| 検証トップ・ワンクリックログイン | `POST /dev/login-as` | 不要（`APP_ENV=production` では404） | `index.tsx`（§11.4・§16.2.1） |
| マイページ | `GET/POST /mypage` | ユーザー | `appPages.tsx`（`mypagePage`） |
| 退会 / ログアウト | `POST /mypage`（`action=withdraw｜logout`）/ `POST /logout` | ユーザー | 同上＋`index.tsx` |
| 退会取消（猶予内） | `POST /admin/users/:id`（`action=cancelWithdraw`） | 管理者 | `appPages.tsx`（`adminUserDetailPage`） |
| Google モック同意〜callback | `GET /auth/google`, `GET/POST /auth/google/consent`, `GET /auth/google/callback` | 不要〜発行後 | `extraPages.tsx`（`googleConsentPage`）/ `authApi` |
| PW リセット申請 / 実行 | `GET/POST /password/reset-request`, `GET/POST /password/reset` | 不要 | `extraPages.tsx`（`passwordResetRequestPage` / `passwordResetPage`） |
| メール変更申請 / 検証 | `GET/POST /mypage/email-change`, `GET /email-change/verify` | ユーザー / 不要 | `extraPages.tsx`（`emailChangePage` / `emailChangeVerifyPage`） |
| 管理者ログイン（一覧＋ワンクリック） | `GET /admin/login` | 不要 | `adminLoginPage.tsx`。パスワードフォームではない（下記注記） |
| 管理者ワンクリックログイン / ログアウト | `POST /dev/login-as-admin` / `POST /dev/logout-admin` | 不要（同上） | `index.tsx`（§11.4） |
| ユーザー一覧 | `GET /admin/users` | 管理者 | `appPages.tsx`（`adminUsersListPage`） |
| ユーザー詳細 / BAN / 解除 / PII操作 | `GET/POST /admin/users/:id`（`action` で分岐） | 管理者 | `appPages.tsx`（`adminUserDetailPage`） |

**`adminLoginPage.tsx` の実態**: 画面名は「管理者ログイン」だが、実装は「シード管理者の一覧を表示し、各行の『ログイン』ボタンでパスワード無しに `POST /dev/login-as-admin` を叩く」検証用ページである。ユーザー側の検証トップ（§16.2）と同じ設計思想を管理者にも適用したもの。管理者パスワードによる認証（`admin_users.password_hash` の照合）は `POST /admin/login`（JSON API）としては実装済みだが、それを入力するフォーム画面は無い。
...
> **設計との差分**: 当初は確定ユーザー一覧・未認証一覧の両方に **「初期状態」「現在の状態」** の2列を並べる仕様だったが、実装では `ListVerificationHomeUseCase` が両方の値（`initialState` / `currentState`）を算出しているものの、`homePage.tsx` が実際に描画しているのは **「現在の状態」列のみ**である。「初期状態」列は現状トップ画面に表示されていない（データは失われていない。UseCase の戻り値には残っている）。

| 列 | 確定ユーザー一覧 | 未認証一覧 |
|---|---|---|
| email | ○ | ○ |
| created / expires / consumed | — | ○（`expires` / `consumed` のみ。`created` は非表示） |
| ~~初期状態~~ | 算出のみ・非表示 | 算出のみ・非表示 |
| **現在の状態** | ○（Resolver 派生。`StateChip` で表示） | ○（同上） |
| **アクション** | ○（下記） | ○（下記・メール認証リンク） |
| 日時表示 | — | `expires` / `consumed` は **JST・分まで**（`formatJst` / `date-fns-tz`。null は `—`） |

**確定ユーザー・アクション列（実装の現況）**:
- Cookie セッションで現在ユーザーを判定（`SessionService.resolveFromUserCookie`。成功パスは KV のみ）
- **当該行のユーザーとしてログイン中**: 「マイページ」リンク + 「PWリセット」+（未消費トークンがあれば）「PW更新」+ 「ログアウト」ボタン（`POST /logout` → PRG で `/` へ）
- **未ログイン、または他ユーザー行（active）**: 「ログイン」→ `POST /dev/login-as`（`userId`）+ 「PWリセット」+（同）「PW更新」。**パスワード再入力なし**でセッション発行し `/` へ PRG（アクションがマイページ｜ログアウトに切り替わる）。Googleのみ identity も password 検証をスキップ（検証用 impersonate）
  - 「PWリセット」は `POST /password/reset-request`（email 固定値で送信）、「PW更新」は未消費 `password_resets.raw_token` がある場合のみ表示され `POST /password/reset` を固定パスワード（`Password123!`）で叩く、検証トップ専用のワンクリック導線（設計時未記載）
...
- 期限切れ行: 「再送」ボタン（`POST /signup/resend`。設計時未記載の画面用エンドポイント。§11.4）
- それ以外: リンク無し＋理由（`消費済み` / `再送で無効化` / `トークン未保管`）
...
> **修正メモ**: 以前は `seed_signup_labels` に `display_name`（アプリが検証完了時に必要とするデータ）が混在しており、上表の「シード固定ラベル」分類と実装が食い違っていた。`display_name` は `signup_verifications` 本体（§6.2）へ移動し、`seed_signup_labels` は本当に検証トップ表示専用の `initial_state_label` / `raw_token` のみを持つ形に揃えた。

### 16.3 ユーザー・マイページ（実装の現況）

表示: `status`, profile（email）, identities（provider 名の一覧のみ）。  
操作: 退会（理由選択＋確認モーダル）、ログアウト。

> **設計との差分**: 当初は「自ユーザーの KV セッション一覧（userAgent / ip / createdAt）」「直近の status events」もマイページに表示する想定だったが、実装の `mypagePage`（`appPages.tsx`）はこれらを表示していない。KV セッション一覧・status events（履歴）の閲覧は現状 **管理者・ユーザー詳細画面（§16.4）のみ**で提供されている。また「パスワードリセット申請へのリンク」「メール変更申請へのリンク」もマイページ本体には無く、検証トップ（§16.2.1）または直接 URL 遷移が導線になっている。退会取消はマイページに無い（管理者のみ）。

### 16.4 管理者・ユーザー詳細

表示: profile（email / display_name）, status, user id, verified_at, 全 status_events（`EventTimeline`）, identities 一覧, 当該ユーザーの全セッション（KV。key / IP / User-Agent / 作成日時）, 最新 BAN（reason_code / reason_text / admin_user_id / 日時）, 未消費のメール変更・PWリセット申請有無。  
操作: BAN / BAN解除フォーム（理由入力、確認モーダル付き）、退会取消（`withdrawn` のときのみ表示）、PII匿名化 / PII削除（`banned` または `withdrawn` かつプロフィールが匿名化済みでない場合のみ表示。確認モーダル付き）。  
...
| `requireUserPage` | 画面。無ければ `/`（検証トップ）へリダイレクト |
...
- [ ] 楽観ロックが「event INSERT・詳細 INSERT・users UPDATE を単一 batch」であり、UNIQUE 違反で batch 全体がロールバックすること（users UPDATE の単独先行がないこと）
...
- [ ] `GET /` が認証不要で、確定ユーザー／未認証の両一覧に **「現在の状態」** 列を表示すること（§16.2）。「初期状態」列は現況未表示（§16.2.1 差分注記）。表示する場合は追加対応が必要
