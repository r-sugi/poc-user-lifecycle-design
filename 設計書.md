...
erDiagram
    ...
    user_status_events ||--o| user_unbans : "0..1"
    admin_users ||--o{ user_bans : "操作者"
    admin_users ||--o{ user_unbans : "操作者"
...
| user_unbans | event_id → user_status_events.id | admin_user_id FK→admin_users, created_at（理由は持たない） |
...
D1 batch 内では「UPDATE 0件」を検知して他 statement を取り消せない。失敗時のズレを **「履歴あり・キャッシュ古い」**（events 再計算で復旧可能）に倒すため、順序は次とする。

1. **単発** `INSERT user_status_events (user_id, seq=expected_seq+1, type, …)`
2. `UNIQUE(user_id, seq)` 違反 → 競合（CAS）として `status_conflict` 409
3. 成功後のみ `db.batch([ 詳細 INSERT（user_withdrawals / user_bans / user_unbans）, UPDATE users SET status=?, last_seq=? WHERE id=? AND last_seq=? ])`
4. users UPDATE の影響行数 0 → 履歴は残っているがキャッシュ未進（競合または部分失敗）。リトライ／events 再読で自己修復可能な方向
5. **D1 成功後**、必要なら Queue に `{ userId, reason }` を投入（セッション破棄が必要な場合）
...
- Body: `{ newEmail }` → 申請前にプロフィール存在で重複チェック → email_change_requests、確認メールログ
...
- 方針 B。セッション自動復旧なし。詳細は `user_unbans`

#### `POST /admin/users/:id/cancel-withdraw`

- 方針 B: withdrawn→active、`withdraw_cancelled` event（`actor_type=admin`）
