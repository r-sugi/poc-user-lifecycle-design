...
export const requireUser: MiddlewareHandler<AppBindings & { Variables: Partial<AuthVariables> }> = async (
  c,
  next,
) => {
...
export const requireAdmin: MiddlewareHandler<AppBindings & { Variables: Partial<AuthVariables> }> = async (
  c,
  next,
) => {
  const { sessionService } = createContainer(c)
